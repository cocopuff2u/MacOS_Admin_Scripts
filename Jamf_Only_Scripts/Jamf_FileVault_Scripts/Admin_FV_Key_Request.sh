...
#
# Purpose: Allow a user to retrieve the FileVault Key for a computer via hostname or serial number.
#
# Requires: API access to Jamf Pro, swiftDialog installed, and the user must have permission to view
...
#   1. Parameter 4: client_id (e.g., "76a4c8fe-bb13-408b-9583-48d02299db4892").
#   2. Parameter 5: client_secret (e.g., "lPynBg-nkrx3FWePgwOGZy-CG6cgUuaLIE1tIhi7Z3jxjdaOeM2259AOerWLDt1h6C").
...
function display_welcome_message () {
    ...
    MainDialogBody="${SW_DIALOG} \
        ...
        --message \"Please enter the device details for which you need to retrieve the FileVault Recovery Key. \n\n You must also provide a reason for retrieving the Recovery Key.\" \
    ...
    selected_option=$(echo $message | awk -F '"SelectedOption" : "' '{print$2}' | awk -F '"' '{print$1}' | sed 's/^ *//' | tr -d '\n')
    device_value=$(echo $message | grep "Device" | awk -F '"Device" : "' '{print$2}' | awk -F '"' '{print$1}')
    reason=$(echo $message | grep "Reason" | awk -F '"Reason" : "' '{print$2}' | awk -F '"' '{print$1}')
    logMe "INFO" "User selected option: $selected_option for device value: $device_value"
    logMe "INFO" "Users Reason: $reason"
}
...
while true; do
    ...
    if [[ "$selected_option" == "Serial Number" ]]; then
         serial_num="$device_value"
         logMe "INFO" "Fetching device ID using serial number: $serial_num"
         Get_Jamf_API_Token
         Get_JAMF_Device_ID_Serial
         FileVault_Recovery_Key_Retrieval
         Invalidate_Token
    else
         logMe "INFO" "Fetching device ID using hostname: $device_value"
         Get_Jamf_API_Token
         Get_JAMF_Device_ID_Hostname
         FileVault_Recovery_Key_Retrieval
         Invalidate_Token
    fi
    ...
    if [[ ! "$ID" =~ ^[0-9]+$ ]]; then
        ...
        $SW_DIALOG \
            ...
            --bannertitle "${SD_WINDOW_TITLE}" \
            --icon "${OVERLAY_ICON}" --iconsize 100 \
            --message "Device inventory not found. \n\nPlease make sure the device hostname or serial number is correct." \
    ...
    $SW_DIALOG \
        ...
        --bannertitle "${SD_WINDOW_TITLE}" \
        --icon "${OVERLAY_ICON}" --iconsize 100 \
        --message "The Recovery Key for $device_value is: <br><br>**$filevault_recovery_key_retrieved**" \
        --messagefont "name=Arial,size=17" \
        --infotext "V ${ScriptVersion}" \
        --width 900 \
        --ontop \
