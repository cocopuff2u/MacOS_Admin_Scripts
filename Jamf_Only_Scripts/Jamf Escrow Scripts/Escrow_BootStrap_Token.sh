...
prompt_password() {
    ...
    passlogic=$(dialog -t "$company_name BootStrap Token Escrow" --icon "$self_service_path" -p --alignment center --message "$message" --textfield "Authenticate with Local Password",secure --button1text "Enter Local Password to Continue" --button2text "Not Now (Remind Tomorrow)")
...
show_success_dialog() {
    dialog --title "Success" --message "Bootstrap token has been successfully escrowed to the Jamf server. Thank You" --icon "$self_service_path" --button "Close" -s
